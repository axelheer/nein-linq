...
namespace NeinLinq.Tests.DynamicQuery
{
    public class OrderByTypedTest
    {
        ...
        {
            _ = Assert.Throws<ArgumentNullException>(() => default(IQueryable<Dummy>)!.OrderBy("Name.Length"));
            _ = Assert.Throws<ArgumentNullException>(() => data.OrderBy(null!));
            _ = Assert.Throws<ArgumentNullException>(() => default(IOrderedQueryable<Dummy>)!.ThenBy("Name"));
